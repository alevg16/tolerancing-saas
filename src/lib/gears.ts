...
function blankResult(family: GearFamily): GearResult {
  return {
    ...
    torque1: 0, Ft: 0, pitchVelocity: 0, Kv: 1,
...
/** Bending + contact estimate shared by cylindrical and (virtual) bevel rating. */
function rateMesh(args: {
  Ft: number; faceWidth: number; module: number; d1: number;
  zForLewis: number; ratioU: number; internal: boolean;
  alphaT: number; alphaTw: number; betaB: number; contactRatio: number; betaDeg: number;
  KA: number; Kv: number; mat1: GearMaterial; mat2: GearMaterial;
}) {
  const { Ft, faceWidth: b, module: m, d1, zForLewis, ratioU, internal, KA, Kv } = args;
  const Y = lewisFormFactor(Math.max(5, zForLewis));
  const load = (Ft * KA * Kv) / Math.max(1e-6, b);
  const sigmaF = load / Math.max(1e-6, m * Y);
  const sigmaFAllow = Math.min(args.mat1.sFlim, args.mat2.sFlim);
  ...
  const ea = Math.max(1.0, args.contactRatio);
  const Zeps = args.betaDeg !== 0 ? Math.sqrt(1 / ea) : Math.sqrt((4 - ea) / 3);
  const u = Math.max(1e-6, ratioU);
  const sign = internal ? -1 : 1;
  const sigmaH =
    ZE * ZH * Zeps * Math.sqrt(((Ft * KA * Kv) / (Math.max(1e-6, d1) * b)) * ((u + sign) / u));
  const sigmaHAllow = Math.min(args.mat1.sHlim, args.mat2.sHlim);
...
function analyzeCylindrical(input: GearInput): GearResult {
  ...
  const rate = rateMesh({
    ...
    alphaT, alphaTw, betaB, contactRatio, betaDeg: input.helixAngle, KA: input.KA, Kv,
    mat1: input.mat1, mat2: input.mat2,
  });
  ...
  return {
    ...
    torque1: T1, Ft, pitchVelocity: v, Kv,
    ...rate,
    notes,
  };
}
...
function analyzeBevel(input: GearInput): GearResult {
  ...
  const rate = rateMesh({
    ...
    alphaT, alphaTw: alphaT, betaB, contactRatio, betaDeg: input.type === "spiral_bevel" ? input.helixAngle : 0,
    KA: input.KA, Kv, mat1: input.mat1, mat2: input.mat2,
  });
...
function analyzeWorm(input: GearInput): GearResult {
  ...
  const sigmaH = ZE * Math.sqrt((Ft2 * input.KA) / (Math.max(1e-6, d2 * b) * Math.cos(alphaN)));
  const sigmaHAllow = input.mat2.sHlim; // wheel (bronze) allowable
  // Indicative wheel bending (Lewis on wheel teeth).
  const Y = lewisFormFactor(z2);
  const sigmaF = (Ft2 * input.KA) / Math.max(1e-6, b * mx * Y);
  const sigmaFAllow = input.mat2.sFlim;
  ...
  return {
    ...
    torque1: T1, Ft: Ft2, pitchVelocity: v1, Kv: 1,
    sigmaF, sigmaFAllow, SF_bending: sigmaF > 0 ? sigmaFAllow / sigmaF : Infinity,
    sigmaH, sigmaHAllow, SF_contact: sigmaH > 0 ? sigmaHAllow / sigmaH : Infinity,
    notes,
  };
